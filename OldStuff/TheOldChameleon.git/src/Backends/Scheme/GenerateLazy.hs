...
module Backends.Scheme.GenerateLazy (
    generate
) where
...
-- 4. laziness is implemented by delaying each value that is bound to a Haskell
-- variable; hence accessing a variable is always via force
...
trDataType ctor (AI.DataType id tys) = 
  ...
  in
  DefBegin 
  ( -- the constructor function (external use, must be curried and delayed!)
  DefSingle basename (delay $ mkConstructor tys [0..])
...
trLetBnd (LetBnd _ id exp) = 
  DefSingle (IT.f id)
	    (ProcedureCall (Variable (SId "delay")) [trExp exp])
  -- cannot use the optimizing delay here because of silly definitions like x=x

trExp :: BI.Exp -> BSA.Exp
trExp (BI.VarExp id) = force $ Variable (IT.f id)
trExp (BI.ConExp id) = force $ Variable (IT.f id)
trExp (BI.LitExp lit) = Literal (trLit lit)
trExp (BI.AppExp si e1 e2) = ProcedureCall (trExp e1) [delay $ trExp e2]
trExp (BI.AbsExp si id exp) = lambda [IT.f id] (trExp exp)
...
trMatches names (Match ps e: ms) =
  ProcedureCall
    ...
    [lambda [SId "fail"]
      (ProcedureCall (Variable (SId "ret"))
		     [trMatch names ps e])]
  :
  trMatches names ms
...
trMatch (id : ids) (p : ps) e =
  case p of
	...
	(ProcedureCall
	 (Variable (SId "equal?"))
	 [force $ Variable id, Literal (trLit lit)])
	(trMatch ids ps e)
	(Just (ProcedureCall
	       (Variable (SId "fail"))
	       []))
...
delay :: BSA.Exp -> BSA.Exp
delay (ProcedureCall (Variable (SId "force")) [e]) = e
delay e = ProcedureCall (Variable (SId "delay")) [e]

force :: BSA.Exp -> BSA.Exp
force e = ProcedureCall (Variable (SId "force")) [e]
